...
if (!class_exists('EDD_Price_Variation_Images') && class_exists('Easy_Digital_Downloads') ) {
	...
	class EDD_Price_Variation_Images {
		...
		public static function add_image_upload($post_id, $key, $args ) {
			...
			echo '	<tr>';
			echo '		<td>';
			if ( isset($variation_images[$key]) ) {
				echo '<img src="'. $variation_images[$key].'" class="edd_variation_image" width="100" height="100"  data-key="'.$key.'">		';
			}
			echo '		</td>';
			echo '		<td>';
			...
			echo '		</td>';
			echo '		<td>';
			echo '			<input name="edd_variation_images['.$key.']"  class="edd_variation_images" data-key="'.$key.'" type="text" value="'.( isset($variation_images[$key]) ? $variation_images[$key] : '' ).'" style="display:inline-block;width:80%" />';
			echo '		</td>';
			echo '		<td>';
			echo '			<button id="upload_image_button"  style="display:inline-block;"  type="button" value="" data-key="'.$key.'">upload</button>';
			echo '		</td>';
			echo '	<tr>';
			echo "</table>";
			...
			if (count($variation_images)>0){
				...
				$html .= "<script>
				            jQuery(document).ready(function($){
                            	...
                            	jQuery('body').on( 'change', '.edd_price_options select' , function() {
                            		jQuery('.edd-ul-price-variation-images li').hide();
                            		jQuery('.edd-ul-price-variation-images li[data-id='+jQuery(this).find(':selected').val()+']').show();
                            	});
                                } , 500);
